...
class AdminProfilePageScreen extends StatefulWidget {
  const AdminProfilePageScreen({Key? key}) : super(key: key);
...
class _AdminProfilePageScreenState extends State<AdminProfilePageScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: Scaffold(
      ...
      body: BlocProvider(create: (_) => userBloc,
          ...
          else if (state is UserLoaded)
            {
              switch (state.userdata.email) {
                case '':
                  {
                    ...
                    return StreamBuilder<DocumentSnapshot>(stream: UserRepository().specificUser(),
                            ...
                            case ConnectionState.done:
                              {
                                if (!asyncSnapshot.hasData) {
                                  ...
                                  var userDocument = asyncSnapshot.data;
                ...
                default:
                  {
                    print('email db');
                    username.text = state.userdata.username;
                    userID.text = state.userdata.userID;
                    email.text = state.userdata.email;
                    telNumber.text = state.userdata.telNumber;
                    isStudent.text = "Admin";
                    return SingleChildScrollView(
                      child: Padding(
                        padding: const EdgeInsets.all(12),
                        child: Card(
